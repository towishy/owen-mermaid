...
title: "Owen Mermaid Plugin Design"
status: adopted
product: "owen-mermaid"
surface: "Obsidian inline toolbar, zoom viewer and visual diagram editor"
kind: plugin
updated: 2026-08-12
...
# Owen Mermaid Plugin Design

## Decision Summary

| 항목 | 결정 |
| --- | --- |
| 사용자 | Obsidian 안에서 Mermaid diagram을 편집·확대·export하는 작성자 |
| Output job | Diagram source와 visual layout을 동기화하고 공유 가능한 PNG/JPG로 export한다 |
| Primary object | Mermaid code block, visual node·edge layout과 export artifact |
| Primary action | 현재 diagram을 visual editor에서 수정하고 source에 반영한다 |
| Composition | Clear icon toolbar + opaque canvas editor + settings panels |
| 핵심 판단 | Mermaid source fidelity와 unsupported syntax 보존을 visual editor 편의보다 우선한다 |

## Context

Owen Mermaid는 rendered block에 inline command를 추가하고 zoom·visual flowchart editing·batch export를 제공한다. Owen metadata comment로 layout을 보존한다.
...
- Inline toolbar는 compact Clear Glass icon dock
- Visual editor canvas와 source preview는 opaque
- Shape·connector·layout mode를 segmented control로 분리
- Settings body는 opaque row
- Korean long-label과 dark mode SVG 검증

### Excluded Or App-Owned

- Unsupported Mermaid line 삭제
- Zoom viewer에서 diagram content 뒤 blur
- Source와 visual editor의 silent divergence

## Design Decisions

### D-01. Source round-trip을 보존한다

- **결정**: 지원하지 않는 style, class, click, subgraph line도 source preview와 save에서 유지한다
- **이유**: Visual editor가 원문 의미를 손실하면 안 된다
- **검증**: Parse/generate round-trip과 unsupported line test를 실행한다

### D-02. Canvas와 command layer를 분리한다

- **결정**: Shape·connector·layout command는 Clear control, diagram substrate는 opaque canvas로 둔다
- **이유**: Node label과 edge 대비가 material effect보다 중요하다
- **검증**: Dense·long-label·Korean diagram의 overflow와 contrast를 확인한다

### D-03. Export outcome을 구조화한다

- **결정**: 각 file의 success, failed, skipped, size와 report path를 표시한다
- **이유**: Batch partial failure를 재실행할 수 있어야 한다
- **검증**: Mixed export fixture와 output report를 검증한다
...
- npm test and typecheck
- Obsidian reading/live preview smoke
- SVG a11y·overflow·dark and raster export checks
